...
#' @param monthly_mean_pdo Mean pdo from springRunDSM::monthly_mean_pdo,
#' @export
apply_straying <- function(year, natural_adults, hatchery_adults, total_releases,
  ...
  in_bay_releases <- hatchery_adults * springRunDSM::hatchery_release_proportion_bay
  in_river_releases <- hatchery_adults * (1 - springRunDSM::hatchery_release_proportion_bay)
...
compute_adult_stray_rates <- function(type = c("natural", "hatchery"), sim_year,
  ...
  new_data <- purrr::map_df(names(springRunDSM::hatchery_to_watershed_lookup), function(x) {

    # prepare initial data
    w <- springRunDSM::hatchery_to_watershed_lookup[x]
  ...
  predictions <- betareg::predict(springRunDSM::hatchery_stray_betareg, newdata = new_data)
  ...
  stray_rates <- new_data |> transmute(
    sim_year = sim_year,
    watershed = springRunDSM::hatchery_to_watershed_lookup[hatchery],
    age = age_unorm,
